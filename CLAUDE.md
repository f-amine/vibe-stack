...
pnpm dev:web                   # just the product app (also dev:marketing, dev:admin)
pnpm init:app                  # interactive first-run wizard (rename + features + .env)
pnpm rename <name>             # rename @vibestack scope + brand (lockfile-safe; run pnpm install after)
